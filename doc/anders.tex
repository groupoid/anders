...
\authorrunning{Namdak Tönpa}
\Copyright{Namdak Tönpa}
...
\keywords{Homotopy Type System, Cubical Type Theory, Modal HoTT}
...
Soundness and completeness link syntax to semantics. Canonicity, normalization, and totality ensure computational adequacy.
...
Soundness is proven via cubical sets (CCHM, 2018).
Awodey, "Type Theory and Homotopy" (2012): Completeness in categorical models of type theory.
Coquand, "A Survey of Constructive Models of Univalence" (2018): Semantic completeness challenges in cubical systems.


\subsection{Canonicity, Normalization and Totality}

Canonicity and normalization hold constructively (Huber, 2017).
Streicher, "Semantics of Type Theory" (1991): Normalization and totality in MLTT models.
Huber, "Canonicity for Cubical Type Theory" (2017): Proof of normalization (implying totality) for CCHM.
Huber, "Canonicity for Cubical Type Theory" (2017): Proved strong normalization for CCHM, adapting logical relations to cubical constructs.

\subsection{Consistency and Decidability}

Consistency follows from the model (Bezem et al., 2014).
Decidability is achieved for type checking (Coquand, 2018).

\subsection{Conservativity and Initiality}

Shulman, "Univalence for Inverse Diagrams and Homotopy Canonicity" (2015): Discusses conservativity in HoTT-style systems.
Hofmann, "Syntax and Semantics of Dependent Types" (1997): Categorical framework for extensions.
Initiality is implicit in the syntactic construction (Awodey, 2012).

\newpage
\section{Conclusion}

In Infinity topos where paths commute, \\
cubes arise --- decomposable, mute.\\
Faces align in fibrant constraint,\\
homotopies traced with structure innate.\\

Identity lifts through Kan’s domain,\\
glued by fillers that witness the same.\\
No law assumed, no axiom imposed ---\\
only constructions, composition enclosed.\\

Higher the paths, coherently bound,\\
define the equivalence, the symmetry found.\\
Contractible witness, a spaces of proofs,\\
where judgment refines computational truth.\\

Thus, in category where cubes unfold,\\
types are the spaces, infinities hold ---\\
not mere sets, but the forms which refined,\\
landscape of a proof in your cubical mind!\\
