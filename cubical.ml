...
let rec extractExp : exp -> string = function
  ...
  | Empty | ERecEmpty _ -> fail "cubicaltt does not have built-in empty type"
  | EUnit | EStar | ERecUnit _ -> fail "cubicaltt does not have built-in unit type"
  | EBool | EFalse | ETrue | ERecBool _ -> fail "cubicaltt does not have built-in bool type"
