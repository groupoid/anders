...
\begin{itemize}
    ...
    \item \textbf{Manual Sigma Triple}: Natural transformations are represented as triples $(\eta, \text{commute}, \star)$, where $\star$ is the unique element of the unit type $\mathbf{1}$. This structure ensures compatibility with the compiler's positional projection matching.
    \item \textbf{Cubical Isomorphism}: The isomorphism is established via the \texttt{YonedaMap} and its inverse \texttt{YonedaInv}. The property $\texttt{Yoneda-β}$ (beta reduction) holds definitionally, while $\texttt{Yoneda-η}$ (isomorphism inverse) is proved by establishing a path between natural transformations.
